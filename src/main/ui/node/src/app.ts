...
declare global {
    interface Window {
        claimApp: ClaimUnderwriterApp;
    }
}

class ClaimUnderwriterApp {
    private baseUrl: string = '';
    private policyModal: HTMLElement | null;
    private helpdeskModal: HTMLElement | null;
    private loader: HTMLElement | null;
    private resultSection: HTMLElement | null;
    private resultContent: HTMLElement | null;
    private lastClaimContext: LastClaimContext | null;
    ...
    private initializeEventListeners(): void {
        ...
        const helpDeskClose = document.getElementById('helpDeskCloseBtn');
        if (helpDeskClose) {
        ...
        // Close error alert modal
        const errorAlertOkBtn = document.getElementById('errorAlertOkBtn');
        if (errorAlertOkBtn) {
            errorAlertOkBtn.addEventListener('click', () => {
    ...
    private async handlePolicySubmission(): Promise<void> {
        const policyId = (document.getElementById('policyId') as HTMLInputElement | null)?.value || '';
        const customerId = ((document.getElementById('customerId') as HTMLInputElement | null)?.value || '').toUpperCase();
        const policyNumber = (document.getElementById('policyNumberModal') as HTMLInputElement | null)?.value || '';
        const policyFileInput = document.getElementById('policyFileUpload') as HTMLInputElement | null;
    ...
    private async handleClaimSubmission(): Promise<void> {
        const policyNumber = (document.getElementById('policyNumber') as HTMLInputElement | null)?.value || '';
        const inputMethod = document.querySelector('input[name="inputMethod"]:checked') as HTMLInputElement | null;
        ...
        try {
            ...
            const patientName = (document.getElementById('patientName') as HTMLInputElement | null)?.value || '';
            ...
            if (inputMethod.value === 'text') {
                const invoiceText = (document.getElementById('invoiceText') as HTMLTextAreaElement | null)?.value || '';
                ...
                const fileInput = document.getElementById('claimFileUpload') as HTMLInputElement | null;
            ...
            if (processingIndicator) processingIndicator.style.display = 'none';
            if (submitBtn) submitBtn.disabled = false;
            this.showErrorMessage('Error while processing the claim, Manual Intervention Required');
        }
    }
    ...
    private openHelpDesk(): void {
        ...
        const claimIdEl = document.getElementById('hdClaimId') as HTMLInputElement | null;
        const policyEl = document.getElementById('hdPolicyNumber') as HTMLInputElement | null;
        ...
        const issueEl = document.getElementById('hdIssue') as HTMLTextAreaElement | null;
        if (issueEl) {
            issueEl.focus();
        }
    }

    private clearHelpDeskTranscript(): void {
    ...
    private toggleConversation(): void {
        ...
        if ((history as HTMLElement).style.display === 'none') {
    ...
    private setAiResponseBox(text: string): void {
        const aiBox = document.getElementById('hdAiResponse');
        if (aiBox) {
    ...
    private appendChatBubble(role: 'user' | 'ai', text: string): HTMLDivElement | null {
        const transcript = document.getElementById('hdTranscript');
        if (!transcript) {
            return null;
        }
    ...
    private setHelpDeskSubmitting(isSubmitting: boolean): void {
        const submitBtn = document.getElementById('hdSubmit') as HTMLButtonElement | null;
        const clearBtn = document.getElementById('hdClear') as HTMLButtonElement | null;
        ...
        if (form) {
            ...
            inputs.forEach((el) => {
                const element = el as HTMLInputElement | HTMLTextAreaElement | HTMLButtonElement;
    ...
    private showHelpDeskTyping(): HTMLDivElement | null {
        const transcript = document.getElementById('hdTranscript');
        if (!transcript) {
            return null;
        }
    ...
    private async submitHelpDesk(): Promise<void> {
        const customerName = (document.getElementById('hdCustomerName') as HTMLInputElement | null)?.value?.trim() || '';
        const claimId = (document.getElementById('hdClaimId') as HTMLInputElement | null)?.value?.trim() || '';
        const policyNumber = (document.getElementById('hdPolicyNumber') as HTMLInputElement | null)?.value?.trim() || '';
        const issueDescription = (document.getElementById('hdIssue') as HTMLTextAreaElement | null)?.value?.trim() || '';
    ...
    private getDecisionBadge(decision?: string): string {
    ...
    private calculateTotalRequested(itemizedDecisions?: ItemizedDecision[]): number {
    ...
    public toggleItemizedServices(): void {
    ...
    public toggleDecisionSummary(): void {
    ...
    public toggleDecisionLetter(): void {
    ...
    private formatLetterWithBullets(letterText: string): string {
    ...
    private resetPolicyForm(): void {
        const policyForm = document.getElementById('policyForm') as HTMLFormElement | null;
        if (policyForm) {
            policyForm.reset();
        }
    ...
    private showErrorMessage(message: string): void {
        ...
        const existing = document.getElementById('_errorPopup');
        if (existing) existing.remove();

        // Overlay
        const overlay = document.createElement('div');
        overlay.id = '_errorPopup';
        ...
        iconWrap.style.cssText = `
            ...
            margin: 0 auto 20px;
        `;
        iconWrap.innerHTML = `<svg width="32" height="32" viewBox="0 0 24 24" fill="none"
        ...
        title.textContent = 'Processing Error';
        title.style.cssText = `margin: 0 0 12px; font-size: 18px; font-weight: 700; color: #111827;`;

        // Message
        const msg = document.createElement('p');
        msg.textContent = message;
        msg.style.cssText = `margin: 0 0 28px; font-size: 15px; color: #4B5563; line-height: 1.6;`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Expose module-level functions to global scope so inline onclick handlers can reach them
    (window as any).closeCloudMenu = closeCloudMenu;
    (window as any).selectCloudProvider = selectCloudProvider;
    (window as any).removeFile = removeFile;
    (window as any).removeClaimFile = removeClaimFile;
});

function initializeClaimInputMethod(): void {
    const textRadio = document.getElementById('inputMethodText') as HTMLInputElement | null;
    const fileRadio = document.getElementById('inputMethodFile') as HTMLInputElement | null;
    ...
    if (textRadio && fileRadio && textSection && fileSection) {
        textRadio.addEventListener('change', () => {
            if (textRadio.checked) {
                (textSection as HTMLElement).style.display = 'block';
                (fileSection as HTMLElement).style.display = 'none';
                (document.getElementById('invoiceText') as HTMLTextAreaElement).required = true;
                (document.getElementById('claimFileUpload') as HTMLInputElement).required = false;
            }
        });

        fileRadio.addEventListener('change', () => {
            if (fileRadio.checked) {
                (textSection as HTMLElement).style.display = 'none';
                (document.getElementById('invoiceText') as HTMLTextAreaElement).required = false;
                (document.getElementById('claimFileUpload') as HTMLInputElement).required = true;

                const fileInput = document.getElementById('claimFileUpload') as HTMLInputElement | null;
                if (fileInput && fileInput.files && fileInput.files.length > 0) {
                    (fileSection as HTMLElement).style.display = 'block';
                } else {
                    (fileSection as HTMLElement).style.display = 'none';
                    resetFileUploadUI();

                    setTimeout(() => {
                        const claimFileUpload = document.getElementById('claimFileUpload') as HTMLInputElement | null;
                        if (claimFileUpload) {
...
function resetFileUploadUI(): void {
    const fileInput = document.getElementById('claimFileUpload') as HTMLInputElement | null;
    const uploadArea = document.getElementById('claimUploadArea');
    const filePreview = document.getElementById('claimFilePreview');

    if (fileInput) fileInput.value = '';
    if (uploadArea) (uploadArea as HTMLElement).style.display = 'block';
    if (filePreview) (filePreview as HTMLElement).style.display = 'none';
}

function initializeClaimFileUpload(): void {
    const fileInput = document.getElementById('claimFileUpload') as HTMLInputElement | null;
    const uploadArea = document.getElementById('claimUploadArea');
    const filePreview = document.getElementById('claimFilePreview');
    ...
    function showClaimFile(file: File): void {
        if (fileName && filePreview) {
            fileName.textContent = file.name;
            (filePreview as HTMLElement).style.display = 'block';
            (uploadArea as HTMLElement).style.display = 'none';
        }
    }
}

function removeClaimFile(): void {
    const fileInput = document.getElementById('claimFileUpload') as HTMLInputElement | null;
    const uploadArea = document.getElementById('claimUploadArea');
    const filePreview = document.getElementById('claimFilePreview');
    const fileSection = document.getElementById('fileInputSection');
    ...
    if (filePreview) (filePreview as HTMLElement).style.display = 'none';
    if (uploadArea) (uploadArea as HTMLElement).style.display = 'block';
    if (fileSection) (fileSection as HTMLElement).style.display = 'none';
    if (fileRadio) fileRadio.checked = false;
}

function initializeFileUpload(): void {
    const fileInput = document.getElementById('policyFileUpload') as HTMLInputElement | null;
    const uploadArea = document.getElementById('uploadArea');
    const fileInfo = document.getElementById('fileInfo');
    ...
    uploadArea.addEventListener('dragover', (e) => {
        e.preventDefault();
        (uploadArea as HTMLElement).style.borderColor = 'var(--color-secondary)';
        (uploadArea as HTMLElement).style.background = 'linear-gradient(135deg, rgba(230, 240, 255, 0.8) 0%, rgba(209, 250, 229, 0.8) 100%)';
    });

    uploadArea.addEventListener('dragleave', (e) => {
        e.preventDefault();
        (uploadArea as HTMLElement).style.borderColor = 'var(--color-primary)';
        (uploadArea as HTMLElement).style.background = 'linear-gradient(135deg, rgba(230, 240, 255, 0.4) 0%, rgba(209, 250, 229, 0.4) 100%)';
    });
    ...
    function handleFileSelect(file: File | null): void {
        ...
        if (fileName && fileInfo) {
            fileName.textContent = file.name;
            (fileInfo as HTMLElement).style.display = 'flex';
            (uploadArea as HTMLElement).style.display = 'none';
        }
    }
}
...
function removeFile(): void {
    ...
    if (fileInput) fileInput.value = '';
    if (fileInfo) (fileInfo as HTMLElement).style.display = 'none';
    if (uploadArea) (uploadArea as HTMLElement).style.display = 'block';
}
...
function selectCloudProvider(provider: string): void {
    closeCloudMenu();
