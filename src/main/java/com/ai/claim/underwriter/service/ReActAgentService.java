...
public class ReActAgentService {
    ...
    public ClaimProcessingResult processWithReAct(ExtractRequest request, String policyNumber, String patientName) {
        ...
        try {
            ...
            for (int iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
                ...
                if (containsFinalAnswer(response)) {
                    ...
                    ClaimEvidence finalEvidence = (lastClaimEvidence != null) ? lastClaimEvidence.get(patientName) : null;
                    if (finalEvidence != null) {
                        ClaimDecision decision = finalEvidence.claimDecision();
                        List<String> reasons = parseReasons(decision.getReasons());
                        JsonNode itemizedDecisions = parseItemizedDecisions(finalEvidence.itemizedDecisions());
                    ...
                    logger.error("Claim evidence not found for patient: {}", patientName);
                    return ClaimProcessingResult.error("Error while processing the claim, Manual Intervention Required");
                }
        ...
        ClaimEvidence fallbackEvidence = (lastClaimEvidence != null) ? lastClaimEvidence.get(patientName) : null;
        if (fallbackEvidence != null) {
            ClaimDecision decision = fallbackEvidence.claimDecision();
            List<String> reasons = parseReasons(decision.getReasons());
            JsonNode itemizedDecisions = parseItemizedDecisions(fallbackEvidence.itemizedDecisions());
        ...
        logger.error("Claim evidence not found for patient: {} after {} iterations", patientName, MAX_ITERATIONS);
        return ClaimProcessingResult.error("Error while processing the claim, Manual Intervention Required");
    }
